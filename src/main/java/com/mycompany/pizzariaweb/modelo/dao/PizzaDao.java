...
public class PizzaDao extends GenericoDAO<Pizza>{
    ...
    public void salvar(Pizza objPizza){
        ...
        save(sql, objPizza.getNome(), objPizza.getPrecoBase(), objPizza.getTamanho(), objPizza.getPizza_borda().getCodigoRecheioBorda());
    }
    
    public void alterar(Pizza objPizza){
        String sql = "UPDATE pizza SET NOME=?, PRECOBASE=?, TAMANHO=?, PIZZA_BORDA=? WHERE CODPIZZA=?";
        save(sql, objPizza.getNome(), objPizza.getPrecoBase(), objPizza.getTamanho(), objPizza.getPizza_borda().getCodigoRecheioBorda(), objPizza.getCodPizza());
    }
    ...
    private static class PizzaRowMapper implements RowMapper<Pizza>{
        ...
        public Pizza mapRow(ResultSet rs) throws SQLException{
            ...
            objPizza.setTamanho(rs.getString("tamanho"));
